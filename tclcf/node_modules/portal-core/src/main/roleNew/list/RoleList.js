...
module.exports = {
    ...
    controller: class RoleList {
            ...
            this.cols = [
                {
                    ...
                    template: `<a ng-href="/#/nav/role/details/{{row.id}}" 
                                  ng-if="$ctrl.currentUser.hasPermission('ROLE_ROLE_READ')"
                                  class="list-row-link" 
                                  ng-bind="row.name"></a>
                               <span ng-if="!$ctrl.currentUser.hasPermission('ROLE_ROLE_READ')" 
                                     ng-bind="row.name"></span>`
                },
                {
                    name: 'roleNew.roleList.cols.description',
                    path: 'description'
                },
                {
                    name: 'roleNew.roleList.cols.permissions',
                    template: "<span>{{'role.roleList.permission_count_label' | translate: {result: row.permissions.length} }} </span>"
                },
                {
                    name: 'roleNew.roleList.cols.organizations',
                ...
                {
                    name: 'roleNew.roleList.cols.applications',
        ...
        changeSort(params) {
            ...
            this.listService.sort = params.sort
