...
module.exports = {
    ...
    controller: class UserList {
        ...
        $onInit() {
            ...
            this.toolbarTitle = 'user.userList.title'

            this.cols = [
                {
                    name: 'user.userCard.username',
                    sortable: true,
                    template: '<a ng-href="/#/nav/user/details/{{row.key}}" class="list-row-link" ng-bind="row.userName"></a>'
                },
                {
                    name: 'user.userList.first_name',
                    path: 'firstName',
                    sortable: true
                },
                {
                    name: 'user.userList.last_name',
                    path: 'lastName',
                    sortable: true
                },
                {
                    name: 'user.userCard.organization',
                    path: 'userOrganizations[0].organizationTypeId'
                },
                {
                    name: 'user.userCard.contact',
                    template: '<span>{{row.userPhones[0].type | startCase}} {{row.userPhones[0].phone | phoneNumber}}</span>'
                },
                {
                    name: 'user.userCard.role',
                    path: 'userRoles[0].name'
                },
                {
                    name: 'user.userList.status',
                    template: '<span translate="{{row.status ? \'user.common.status_active\' : \'user.common.status_inactive\'}}"></span>',
                    sortable: true
                }
            ]
